...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    @Query(
        value = "select employee from Employee employee left join fetch employee.enterprise left join fetch employee.manager",
        countQuery = "select count(employee) from Employee employee"
    )
    Page<Employee> findAllWithToOneRelationships(Pageable pageable);

    @Query("select employee from Employee employee left join fetch employee.enterprise left join fetch employee.manager")
    List<Employee> findAllWithToOneRelationships();

    @Query(
        "select employee from Employee employee left join fetch employee.enterprise left join fetch employee.manager where employee.id =:id"
    )
